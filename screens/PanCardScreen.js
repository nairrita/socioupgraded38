...
getAllBlogs = ()=>{
    this.blogRef = db.collection("pan_card_blogs").where("category","==",this.state.category)
    .onSnapshot(snapshot=>{
        var pancardBlogs = snapshot.docs.map(document=>document.data)
        this.setState({
            pancardBlogs : pancardBlogs
        })
    })
}
    ...
    render(){
        return(
            <View style = {styles.container}>
                ...
                <View style = {{flex:1}}>
                    {
                        this.state.pancardBlogs.length===0
                        ?(
                            <View>
